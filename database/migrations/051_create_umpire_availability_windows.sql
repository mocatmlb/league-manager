...
CREATE TABLE IF NOT EXISTS `umpire_availability_windows` (
    ...
    `umpire_user_id` INT UNSIGNED NOT NULL,
...
CREATE INDEX `idx_umpire_availability_user_window` ON `umpire_availability_windows` (`umpire_user_id`, `starts_at`, `ends_at`);
CREATE INDEX `idx_umpire_availability_window_lookup` ON `umpire_availability_windows` (`starts_at`, `ends_at`);
